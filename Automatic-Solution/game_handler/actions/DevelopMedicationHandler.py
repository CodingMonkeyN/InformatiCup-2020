...
class DevelopMedicationHandler():
    action = {"type": "developMedication"}
    ...
    def defaultHandling(self):
        ...
        pathogens = action_dependency_solver.determineBestPathogenToDevelopMedicationFor()
    ...
    def checkIfPathogenIsViable(self, pathogen):
        for pathogen_with_cities in self.game_object.extracted_game_information.pathogen_with_cities:
            if pathogen_with_cities["name"] == pathogen and pathogen_with_cities["infectedCities"] == 0:
                return False
        return pathogen not in self.game_object.extracted_game_information.pathogens_with_medication_in_development and not pathogen in self.game_object.extracted_game_information.pathogens_with_medication_available
